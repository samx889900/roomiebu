                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:justify-between gap-4">
                    <div className="flex-1 min-w-0 w-full">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        <Badge variant="outline" className="truncate max-w-full">{enumToLabel(report.reason)}</Badge>
                      </div>
                      ...
                      {report.targetUser && (
                        <p className="text-sm truncate"><span className="text-muted-foreground">Target user:</span> {report.targetUser.name}</p>
                      )}
                      {report.targetListing && (
                        <div className="flex flex-col gap-1 mb-2 min-w-0">
                          <p className="text-sm truncate"><span className="text-muted-foreground">Target listing:</span> {report.targetListing.title}</p>
                      ...
                      {report.notes && <p className="text-xs text-muted-foreground mt-2 break-words">{report.notes}</p>}
                      <p className="text-xs text-muted-foreground/60 mt-2">{formatRelativeDate(report.createdAt)}</p>
                    </div>
                    {!report.isResolved && (
                      <div className="flex sm:flex-col flex-row flex-wrap gap-2 sm:w-auto w-full">
                        <Button size="sm" variant="outline" className="gap-1 flex-1 sm:flex-none" onClick={() => handleResolve(report.id)}>
                          <Check className="w-3 h-3" /> Resolve
                        </Button>
                        {report.targetUserId && (
                          <>
                            <Button size="sm" variant="outline" className="gap-1 text-amber-400 flex-1 sm:flex-none" onClick={() => handleSuspend(report.targetUserId)}>
                              <Shield className="w-3 h-3" /> <span className="sm:inline">Suspend</span>
                            </Button>
                            <Button size="sm" variant="outline" className="gap-1 text-destructive flex-1 sm:flex-none" onClick={() => handleBan(report.targetUserId)}>
                              <Ban className="w-3 h-3" /> <span className="sm:inline">Ban</span>
                            </Button>
                          </>
                        )}
                      </div>
